<?php include('../header.php');?>
    ...
    $('.submitForm').on('click', function(event) {
        ...
        if (name === "" || firma === "" || mail === "" || phone === "" || alan === "" || detay === "" || not === "") {
        ...
        $.ajax({
            url: 'kurumsal/MailManager/IcerikSubmit.php',
            method: 'POST',
            data: {
                name: name,
                firma: firma,
            ...
            success: function(response) {
                if (response == 1) { // "==" ile eşitlik kontrolü yap
                    alert("İçerik izni talebiniz alınımştır. En kısa sürede dönüş yapılacaktır.");
                    location.reload();
                } else {
                    alert(response);
                }
